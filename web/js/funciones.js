 ...
 // var solicitud = $('input[name=fksol]').val();
  krajeeDialog.confirm("Está por asignar este docente, está seguro de continuar?", function (result) {
    if (result) { // ok button was pressed
      $.post('/se-asesor/asignar', { _csrf: csrfToken, datos: { docenteID: ID, cicloID: ciclo } })
        .done(function (d) {
          if (d !== '0') {
...
function asignacionEstudiante(matricula) {
 ...
 // var solicitud = $('input[name=fksol]').val();
  krajeeDialog.confirm("Está por asignar este estudiante, está seguro de continuar?", function (result) {
    if (result) { // ok button was pressed
      $.post('/tututor/asignar-tutorado', { _csrf: csrfToken, datos: { estudianteID: matricula, tutor: tutor, ciclo: ciclo } })
        .done(function (d) {
          if (d !== '0') {
            var datos = JSON.parse(d);
            krajeeDialog.alert('Se ha asignado tutorado al tutor ' + datos.matricula);
...
//funciones ajax borra prestador
function borrarPrestador(ID) {
  krajeeDialog.confirm("<h6>Está usted seguro borrar al prestador " + ID + "? </h6>", function (result) {
    if (result) { // ok button was pressed
      $.post('/se-prestador/borrar-prestador', { _csrf: csrfToken, datos: { ID: ID } })
        .done(function (d) {
          if (d !== '0') {
            var datos = JSON.parse(d);
            krajeeDialog.alert('Se elimino correctamente ' + datos.ID );
...
function iniciar() {
  krajeeDialog.confirm("<h6>Está usted seguro borrar al tutor " + ID + "? </h6>", function (result) {
    if (result) { // ok button was pressed
      $.post('/se-planeacion/iniciar-planeacion', { _csrf: csrfToken, datos: { ID: ID } })
        .done(function (d) {
          if (d !== '0') {
            var datos = JSON.parse(d);
            krajeeDialog.alert('Se elimino correctamente ' + datos.ID );
...
function borrarTutor(ID) {
  krajeeDialog.confirm("<h6>Está usted seguro borrar al tutor " + ID + "? </h6>", function (result) {
    if (result) { // ok button was pressed
      $.post('/se-asesor/borrar-tutor', { _csrf: csrfToken, datos: { ID: ID } })
        .done(function (d) {
          if (d !== '0') {
            var datos = JSON.parse(d);
            krajeeDialog.alert('Se elimino correctamente ' + datos.ID );
